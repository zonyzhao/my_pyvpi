...
{    
    ...
    Py_DECREF(self->obj);
    self->obj    = PyUnicode_FromString("");
    self->_vpi_value.value.str = PyUnicode_AsUTF8(self->obj);
    // zony modifiy
...
void pyvip_value_update_value(s_pyvpi_value *self, s_vpi_value *ovalp, PLI_INT32 blen)
{
    ...
    case vpiStringVal:
        if(self->fixed_handle == NULL) {
            if(self->obj == NULL)
                self->obj    = PyUnicode_FromString(ovalp->value.str);
            self->_vpi_value.value.str = PyUnicode_AsUTF8(self->obj);
        } 
        else {
    ...
    case vpiScalarVal:
        if(self->fixed_handle == NULL) {
            self->obj    = PyLong_FromLong(ovalp->value.scalar);
            self->_vpi_value.value.scalar = PyLong_AsLong(self->obj);
        }
        else {            
    ...
    case vpiIntVal:
        if(self->fixed_handle == NULL) {
            self->obj    = PyLong_FromLong(ovalp->value.integer);
            self->_vpi_value.value.integer = PyLong_AsLong(self->obj);
        }
        else {
    ...
    case vpiVectorVal:
        if(self->fixed_handle == NULL) {
            ...
            dictobj = PyDict_New();
            PyDict_SetItem(dictobj,PyUnicode_FromString("size"),PyLong_FromLong(blen));
...
PLI_INT32 update_format(p_pyvpi_value self, PLI_INT32 nformat, PyObject* nobj)
{
    ...
    case vpiStringVal:
        if(nobj == NULL)
            nobj    = PyUnicode_FromString(" ");
        if (! PyUnicode_Check(nobj)) {
                PyErr_SetString(VpiError,
                                "The value must be a string<Current format is string>.");
                return -1;
            }
        if(self->fixed_handle == NULL) {
            self->_vpi_value.value.str = PyUnicode_AsUTF8(nobj);
        }
        else {
            char *ptr = PyUnicode_AsUTF8(nobj);
            int  i = 0;
            if(PyUnicode_GET_LENGTH(nobj) + 1 > self->cache_size) {
                PyErr_SetString(VpiError,
    ...
    case vpiScalarVal:
        if(nobj == NULL)
            nobj = PyLong_FromLong(vpi0);
        if (! PyLong_Check(nobj) || PyLong_AsLong(nobj) < 0 || PyLong_AsLong(nobj) > 3) {
            PyErr_SetString(VpiError,
                            "The value must be vpi[0,1,X,Z]<Current format is vpiScalarVal>.");
            return -1;
        }
        self->_vpi_value.value.scalar = PyLong_AsLong(nobj);
        break;
    case vpiIntVal:
        if(nobj == NULL)
            nobj    = PyLong_FromLong(0);
        if (! PyLong_Check(nobj)) {
                PyErr_SetString(VpiError,
                                "The value must be an int<Current format is vpiIntVal>.");
                return -1;
            }
        self->_vpi_value.value.integer = PyLong_AsLong(nobj);
        break;
    case vpiRealVal:
        if(nobj == NULL)
